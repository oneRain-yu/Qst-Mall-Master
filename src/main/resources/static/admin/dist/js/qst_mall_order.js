$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        postData: {
            paramOrderNo : $('#paramOrderNo').val(),
            paramOrderStatus : $('#paramOrderStatus').val(),
        ...
        colModel: [
            ...
            {label: '售后状态', name: 'afterSaleStatus', index: 'afterSaleStatus', width: 80, formatter: afterSaleStatusFormatter},
            {label: '售后物流', name: 'afterSaleTransNo', index: 'afterSaleTransNo', width: 80, formatter: afterSaleTransNoFormatter},
    ...
    function afterSaleStatusFormatter(cellvalue, rowObject) {
        //售后状态
        if (cellvalue == '' || cellvalue == null) {
            return "无";
        }
        if (cellvalue == 'APPLY') {
            var html = "申请售后<br>（<a href=\'##\' onclick=toVerifyAfterSale(" + rowObject.rowId + ")>审核</a>）";
            return html;
        }
        if (cellvalue == 'WAIT_RECEIVE') {
            return "等待退货物流<br>（<a href=\'##\' onclick=toConfirmReceive(" + rowObject.rowId + ")>确认收货</a>）";
        }
        if (cellvalue == 'FINISH') {
            return "退货结束";
        }
        if (cellvalue == 'REFUSED') {
            return "拒绝退货";
        }
    }

    function afterSaleTransNoFormatter(cellvalue) {
        //售后状态
        if (cellvalue == '') {
            return "无";
        }
...
function toVerifyAfterSale(orderId){
    swal({
        title: "售后审核",
        icon: "warning",
        buttons: {
        ...
        var accept = flag ? 1 : 0;
        var acceptStr = flag ? '同意退货' : '拒绝退货';
        $.ajax({
            type: 'POST',
            url: '/admin/orders/aftersale/accept',
            data: {
                "orderId": orderId,
                "accept" : accept
            },
...
function toConfirmReceive(orderId){
    swal({
        ...
        if (flag){
            $.ajax({
                type: 'POST',
                url: '/admin/orders/aftersale/receive/confirm',
                data: {
                    "orderId": orderId
                },
                success: function (result) {
